...
def init_db():
    ...
    # Verificar si existe usuario gruporevi
    response = supabase.table('usuarios').select('*').eq('username', 'gruporevi').execute()
    if not response.data:
        # Crear usuario gruporevi con contraseña revigrupo1
        ...
        # Si ya existe, aseguramos que la contraseña sea la correcta (opcional)
        # Podrías añadir lógica para actualizar contraseña si es necesario
        pass
...
def obtener_ultimo_cuadre(sucursal, fecha=None):
    """
    Obtiene el último cuadre de la sucursal, opcionalmente con fecha <= fecha dada.
    Si fecha es None, obtiene el último cuadre de cualquier fecha.
    Retorna diccionario con 'existe' y 'fondo_siguiente'.
    """
    query = supabase.table('cuadres').select('*').eq('sucursal', sucursal)
    if fecha:
        query = query.lte('fecha', fecha)  # fechas menores o iguales a la seleccionada
...
def obtener_totales_previos(sucursal, fecha, turno_actual):
    """
    Obtiene la suma de totales de turnos anteriores para la misma fecha y sucursal.
    Si turno_actual es "Tarde", suma los turnos "Mañana".
    Si es "Completo" o "Mañana", no suma nada.
    """
...
def obtener_historial(fecha_inicio=None, fecha_fin=None, sucursal=None):
    """Obtiene los cuadres con filtros opcionales."""
    ...
    if response.data:
        for user in response.data:
            ...
            with col4:
                ...
                st.write(estado)
            with col5:
                # No permitir desactivar al propio admin (gruporevi) ni al usuario actual
...
def main_app():
    ...
    with st.sidebar:
        ...
        # Panel de administración solo para admin (solo usuarios)
    ...
    if modo == "📋 Nuevo cuadre":
        ...
        with col1:
            ...
            # REF INDEX
            # --- Fondo inicial automático con memoria entre días ---
            # Obtener el último cuadre de la sucursal (puede ser de días anteriores)
            ...
            if 'checkbox_estado' not in st.session_state:
                st.session_state.checkbox_estado = True

            # Valor sugerido (si hay cuadre anterior)
            fondo_sugerido = 0.0
            fecha_origen = ""
            turno_origen = ""
            ...
            # Lógica de habilitación del campo
            if turno == "Tarde" and hay_cuadre_anterior:
                # Turno tarde con cuadre anterior: forzamos el valor, sin checkbox, campo deshabilitado
                ...
                if hay_cuadre_anterior:
                    # Hay cuadre anterior: mostramos checkbox
                    ...
                    if usar_mismo:
                        ...
                        # Checkbox desmarcado: habilitamos campo para ingreso manual
                        campo_deshabilitado = False
                else:
                    # No hay cuadre anterior: campo habilitado, sin checkbox
                    campo_deshabilitado = False

            # Mostrar campo numérico (siempre visible)
            ...
            # Actualizar session_state solo si el campo está habilitado (para capturar cambios manuales)
        ...
        if st.button("🚀 Calcular cuadre", type="primary"):
            with st.spinner("Obteniendo facturas de Alegra y calculando..."):
                ...
                if not facturas:
                    ...
                    # Obtener totales de turnos anteriores si es necesario
                    ...
                    with st.expander("📊 Ver detalle de ventas por método de pago (turno actual)"):
                        detalle = {
